...
SELECT SUM(SALARY) AS 'Total de salários'
FROM hr.employees;

SELECT
	ROUND(MAX(SALARY), 2) AS 'Maior salário',
    ROUND(MIN(SALARY), 2) AS 'Menor salário',
    ROUND(SUM(SALARY), 2) AS 'Total de salários',
    ROUND(AVG(SALARY), 2) AS 'Média salarial'
FROM hr.employees;

SELECT COUNT(*) AS 'Pessoas Programadoras'
FROM hr.employees
WHERE JOB_ID LIKE 'IT_PROG';

SELECT JOB_ID, SUM(SALARY) AS 'Total de salários'
FROM hr.employees
GROUP BY JOB_ID;
...
SELECT
	FIRST_NAME,
    LAST_NAME,
    DATEDIFF(CURRENT_DATE, HIRE_DATE) AS 'Dias na empresa'
FROM hr.employees;
